...
def split_text_into_instances(document, tokenizer, max_tokens=1024):
    tokens_batch = tokenizer.batch_encode_plus(document["text"], add_special_tokens=True)['input_ids']
    instances_decoded = []
...
def process_document(document, language_detector_path, language_detector_model, consecutive_threshold):
    ...
    for instance in document:
        ...
        if instance_results:
            instance_label, instance_words, instance_tags, instance_groups, instance_languages = instance_results
...
def bilingual_detection(num_workers, dataset, language_detector_path,
                        language_detector_model, consecutive_threshold):
...
def translation_detection(dataset):
    ...
    results_dict = {'embedded_label': embedded_label_list, 'primary_label': primary_label_list,
                    'embedded_sentence': embedded_sentence_list, 'primary_sentence': primary_sentence_list,
                    'instance_index': instance_index_list, 'document_index': document_index_list}
    return Dataset.from_dict(results_dict)
...
def main():
    ...
    pattern = r'\.\w+'
    output_filename = re.sub(pattern, '', args.filename).replace("./", "").replace("/", "___")

    if args.repo_id:
        if args.cache_dir:
            file_path = hf_hub_download(repo_id=args.repo_id, repo_type="dataset", filename=args.filename,
    ...
    if 'instance_text' not in dataset.column_names:
        instances_dataset = dataset.map(
            lambda document: split_text_into_instances(document, tokenizer, args.max_tokens),
            batched=True, batch_size=1000, num_proc=1, remove_columns=dataset.column_names,
        ...
        print("Finished extracting instances. Instances dataset saved at " + results_folder + "/instances")
    else:
        print("Instances column found. Skipping instance extraction.")

    if 'instance_labels' not in dataset.column_names:
        bilingual_dataset = bilingual_detection(args.num_workers, dataset, coswid_path, coswid_model, args.N)
        bilingual_dataset.save_to_disk(results_folder + "/bilingual", num_shards=1)
        dataset = bilingual_dataset
        print(
            "Finished classifying instances. Instance classification dataset saved at " + results_folder + "/bilingual")
    else:
        print("Bilingual classification labels column found. Skipping instance classification.")
    ...
    print(f"Found {num_bilingual_instances} bilingual instances out of {num_total_instances} total instances "
    ...
    print(
        "Finished finding translation pairs. Translation pairs dataset saved at " + results_folder + "/translation")

    print(f"Found {num_translation_instances} translation instances out of {num_total_instances} total instances "
