...
def get_similarity_score(embeddings1, embeddings2):
    ...
    similarity_matrix = torch.matmul(
        F.normalize(embeddings1, p=2), F.normalize(embeddings2, p=2).transpose(0, 1)
    )
...
def detect_translations(
    sentences_embedded, sentences_primary, tokenizer, model, threshold=0.6
):
...
def sentence_breaker(words, labels):
    ...
    for sentence in sentences:
        ...
        sentence_labels = [
            labels[words.index(word)] for word in sentence_words if word in words
        ]
...
def extract_embedded_and_primary_sentences(sentences, sentence_labels):
    ...
    embedded_label = label_counts.most_common(2)[1][0]
...
def get_min_edit_distance_filter(
    sentence_1, sentence_2, min_edit_distance, min_edit_distance_ratio
):
    ...
    return (
        ...
        and edit_distance_ratio >= min_edit_distance_ratio
    )
...
def apply_filters(
    ...
    token_length_filter = (
        min_length <= len_tokens_1 <= max_length
        and min_length <= len_tokens_2 <= max_length
    )
    max_length_ratio_filter = (
        max(len_tokens_1 / len_tokens_2, len_tokens_2 / len_tokens_1) <= max_ratio
    )
    min_edit_distance_filter = get_min_edit_distance_filter(
        sentence_1, sentence_2, min_edit_distance, min_edit_distance_ratio
    )
    language_detection_filter = get_language_detection_filter(sentence_1, sentence_2)
    return (
        token_length_filter
        and max_length_ratio_filter
        and min_edit_distance_filter
        and language_detection_filter
    )
